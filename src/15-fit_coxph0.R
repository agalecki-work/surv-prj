...
if (ntvarsx == 2) message("coxph using `train_data` wout tt vars ") else message("coxph using `train_fgdata` wout tt vars")
...
# multiple coxph using `train_data` or `train_fgdata wout tt vars
 ...
 #  cox0_xfits <- cox0_FITs
  ...
  cox0_FITs_glance0 <- plyr::rbind.fill(list_of_glances)
  rnms <- rownames(mod_cxterms_mtx)
  cox0_FITs_glance <- bind_cols(data.frame(model=rnms ),cox0_FITs_glance0)  
